...
export (Color) var flash_cure_color = Color.greenyellow
...
func _flash_cure():
	for i in range(N_FLASHES * 2):
		var color = health_over.tint_progress if i % 2 == 1 else flash_cure_color
		var time = FLASH_RATE * i + FLASH_RATE
		flash_tween.interpolate_callback(health_over, time, "set", "tint_progress", color)
	flash_tween.start()
...
func _on_Player_healt_updated(health):
...
func _on_Player_max_health(max_health):
	float(local_max_health)
	local_max_health = max_health / 4


func _on_Enemy_healt_updated(health):
	max_health = health
	health_over.value = health
	update_tween.interpolate_property(health_under, "value", health_under.value, health, 0.4, Tween.TRANS_SINE, Tween.EASE_IN_OUT, 0.4)
	update_tween.start()
...
func _on_Enemy_max_health(max_health):
	float(local_max_health)
	local_max_health = max_health / 4


func _on_Snake_healt_updated(health):
	health_over.value = health
	update_tween.interpolate_property(health_under, "value", health_under.value, health, 0.4, Tween.TRANS_SINE, Tween.EASE_IN_OUT, 0.4)
	update_tween.start()
...
func _on_Snake_max_health(max_health):
	float(local_max_health)
	local_max_health = max_health / 4


func _on_HealthOver_value_changed(value):
	value = max_health
